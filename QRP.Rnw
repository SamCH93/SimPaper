...
follow their protocol in the execution phase. However, if no protocol is in
place, they can engage in various QRPs in order to increase the chance for
showing superiority of their preferred method.
...
adapt the mechanism until they find conditions where their method is
superior (E1). For example, \dots
...
also be included in the simulation study. \dots
...
If no a priori size calculation has been conducted, researchers can choose the
...
strategies until their favored method looks superior.
...
their favored method performed best (R1). Not mentioning the conditions in which
...
hypothesized a priori (R2), this leads to over-confidence in the method.

Failing to report Monte Carlo uncertainty (R3), \eg error bars or confidence
intervals, is a QRP with various consequences. It confounds the research
consumers ability to assess the quality of evidence from the simulation study.
It also allows the research to present random differences between methods
...
To illustrate the QRPs from Table~\ref{table:QRPs}, we designed and conducted
a simulation study evaluating the ``adaptive importance elastic net'' (\ainet).
\ainet{} computes weights for an adaptive elastic net via random forest importance
measures. Superficially, the proposed method seems sensible. However, in a
linear DGP, no advantage over the classical adaptive elastic net is expected.
For details, the reader is referred to the simulation protocol
\citep{pawel2022protocol}.

\paragraph{Altering the DGP (E1)}
The simulation study was planned with a low- to high-dimensional linear-logistic
DGP. In none of the scenarios we could detect a significant performance increase
by using the elastic net over standard logistic regression or elastic net.
\todo{Find good setting. Figure}

\paragraph{Switchting the primary estimand (E4)}
Specifying a \emph{single} or combined primary estimand is important not only
for planning the size of the simulation study. The Brier score was chosen as the
primary estimand in our protocol. However, we compute and report other proper
scores, such as the log-score, and discriminatory measures, such as AUC and
accuracy.
\todo{Find good setting. Figure}

\paragraph{Removing comparators (E3)}
Comparing against appropriate competitors is vital to judge the benefits of the
novel method. We compared \ainet{} against standard logistic regression, random
forests, the standard elastic net and adaptive elastic net. However, we observed
only minor (if any) improvements of \ainet{} over the elastic net. If there was no
protocol, we could decide to omit the comparisons with the elastic net by
arguing that solely the comparison among adaptive methods (AEN \vs \ainet) was
sought.
\todo{Find good setting. Figure}

\paragraph{Altering the scope of the simulation study (R2)}
Instead of minor tweaks to the DGP, simulation parameters or methods, the target
of the simulation study could be altered entirely. For \ainet, we focused purely
on predictive performance (together with calibration). However, we could just as
easily focus on discrimination (\eg AUC) or estimation performance (\eg MSE of
the estimated coefficients).
\todo{Find good setting. Figure}

\paragraph{Handling exceptions differently (E6)}
In our protocol, we clearly specified how difficult it would be to judge the
results of simulation settings in which more than 10\% of the runs failed to
converge. We decided to omit these settings. However, without a pre-specified
protocol, we could instead impute the missing scenarios with the ``worst-case''
...
imputation scheme yields results unreasonably in favor of \ainet. Also note that
the resulting confidence intervals ignore the imputation uncertainty and thus
underestimate $p$-values.
\todo{Find good setting. Figure}

\paragraph{Arbitrarily choosing the simulation size (E5)}
A thorough planning of the simulation size in terms of expected precision of the
primary estimand is crucial. An arbitrarily chosen (but often too small)
simulation size yields more heterogeneous results, but can be executed faster.
The danger of seeking ``a good random seed'' is large and the results may exert
strong bias in favor of the novel method. 
\todo{Find good setting. Figure}

\section{Discussion and recommendations} \label{sec:discussion}
We have \dots

Table~\ref{table:recommendations} shows our recommendations for improving the
quality of comparative simulations studies, and prevent QRPs.
\begin{table}[!htb]
