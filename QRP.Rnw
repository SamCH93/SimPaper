...
made-up method as an improvement over others if QRPs remain undisclosed
(Section \ref{sec:study}). We also use the same example to show how
over-optimism can be avoided if a sound methodological route is taken instead.
Similar studies have been conducted by \citet{Niessl2021} and
\citet{Jelizarow2010}, and by \citet{Simmons2011} in the context of $p$-hacking
in psychological research, and were the main inspiration for this work. Finally,
we provide simple and concrete suggestions for researchers, reviewers, and other
academic stakeholders to improve the methodological quality of comparative
simulation studies (Section \ref{sec:discussion}).
...
categorized with respect to the stage of research at which they can occur. Th
...
simulation size in terms of expected precision of the primary estimand is
important. An arbitrarily chosen (but often too small) simulation size yields
more heterogeneous results, but can be executed faster. By failing to conduct a
...
strategies until their favored method looks superior. Choosing the wrong
strategy can introduce systematic bias and misleading conclusions.
...
It also allows the researcher to present random differences in performance as if
...
the classical adaptive elastic net is expected. For more details on the wokrings
of the method, we refer to the simulation protocol \citep{pawel2022protocol}.
...
variable (EPV). \todo{Make Figure: rho = 0.95 original -> rho 0.95, sparsity =
...
\ainet). \todo{remove EN figure}

\subsection{Altering the objective of the simulation study (R2)}
% Instead of tweaking the data generating process, the simulation parameters or
% the comparati
% methods, the target of the simulation study could be altered entirely.
The objective of the simulation study was to evaluate the overall predictive
performance of \ainet{} as from a theoretical perspective this is the domain
where the method could lead to improvements. We did, however, not find any
improvements over comparator methods, so one possibility is to look for an
entire differnet objective where \ainet{} performs better. For instance, we
could switch the focus on discrimination (\eg area under the curve) or
estimation performance (\eg mean squared error of the estimated coefficients).
...
that these settings will thus be omitted. However, without a pre-specified
handling of these situations, we could have tweaked the inclusion/exclusion
...
The previous sections painted a mostly negative picture on how undisclosed QRPs
...
grouped with respect to which stakeholder they concern.
...
\citep{Simmons2011}. Since it is easier to publish novel and superior methods
\citep{Boulesteix2015}, methodological researcher will typically desire to show
the uniform superiority of a method rather than to disclose its strength's and
weaknesses.
...
and exploratory findings, and it lowers the risk of flawed methods being
promoted as improvement. This step may at first it seem as a disadvantage for an
individual researcher as it leads to more work and lower possibility of
publication. However, it also provides researchers a means to differentiate
themselves and to think harder about setting up a good simulation study.
...
are set in place. This approach can also make making post hoc modifications of
the simulation study more legitimate. Finally, it is important for researchers
to separate exploratory from confirmatory findings in the reporting of their
simulation studies.
