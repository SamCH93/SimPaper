...
competitors they can adapt the data-generating mechanism (E1) \dots

If researchers are unable to find data-generating mechanisms under which their
favored method beats a competitor method, they can exclude the competitor from
the comparison (E2). Similarly, to make the favored method look better,
...
researchers realizes that their favored method performs worse in predition
tasks.
