...
the classical adaptive elastic net is expected. For more details, we refer to
the simulation protocol \citep{pawel2022protocol}.
...
regression for the combination of a only few variables being associated with the
outcome (high sparsity), a non-linear effect, and a low number of events per
variable (EPV). \todo{Find good setting. Figure}
...
\subsection{Altering the scope of the simulation study (R2)}
...
\subsection{Arbitrarily choosing the simulation size (E5)}
