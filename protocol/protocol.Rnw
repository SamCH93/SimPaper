...
\end{align*}
once per simulation. The chosen parameter values were inspired by previous
simulation studies and should resemble realistic mechanisms occurring in
practice. Finally, the simulation parameters are varied fully factorially in the
following way:
...
occurring in practice. Note that it is usually recommended to choose the sample
size based on the implied number of events together with the number of covariates
in the model \citep{vanSmeden2018, Riley2018}. We will use this approach in
...
\begin{itemize}
  ...
  $$\BS = B^{-1} \sum_{b=1}^B n_{\text{test}}^{-1} \sum_{i=1}^n (y_{ib} - \hat{y}_{ib})^2,$$
  ...
  \begin{itemize}
    \item Scaled Brier score on independently generated test data,
    $$\BS^{*} = 1 - \BS/\BS_{0}$$
    ...
    $$\LS = - B^{-1} \sum_{b=1}^B n_{\text{test}}^{-1} \sum_{i=1}^n \left\{ y_{ib} \log(\hat{y}_{ib})
    ...
    \item Calibration slope $\hat b$ on independently generated test data, from
          the model
    $$\logit\Ex[Y \given \rx^\top\hat\shiftparm] = a + (b - 1)\logit(\hat{y})$$
    ...
    \item Calibration in the large $\hat a$ on independently generated test
          data, from the model
    $$\logit\Ex[Y \given \rx^\top\hat\shiftparm] = a + \logit(\hat{y})$$
...
$$B =V/(n_{\text{test}} \cdot \sqrt{0.001}).$$
...
\begin{itemize}
  \item Standard logistic regression will not work when $p \geq n$ and we will
  therefore exclude it from these simulation conditions
  \item If convergence is impossible for a  method in a certain
  simulation, the simulation will be excluded
