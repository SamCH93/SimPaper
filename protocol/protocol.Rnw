...
considered, reflecting rare to common events.
...
in the simulation study, especially in high-dimensional settings. Moreover,
standard logistic regression cannot even work in these cases. We will only
take simulations into account that successfully converged? \todo{what here?}
