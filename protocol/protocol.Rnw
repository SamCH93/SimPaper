...
simulation studies and are supposed to resemble realistic mechanisms occurring
in practice \todo{citations of other studies}. Finally, the simulation
parameters are varied fully factorially in the following way:
...
\begin{itemize}
  ...
  of events is close to the underlying probabilities. The Brier score is a proper scoring rule, meaning that it is minimized if a predicted distribution is equal to the data-generating distribution
  ...
  \begin{itemize}
    ...
    \item Calibration slope $\hat b$ on independently generated test data. The
    calibration slope $\hat b$ is obtained by regressing
