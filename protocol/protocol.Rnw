...
in order to find simulation conditions and evaluation metrics which favor
\ainet{} over other methods.
...
We use \pkg{pROC} version 1.17.0.1 to compute the AUC \citep{pkg:proc}.
Random forests are fitted using \pkg{ranger} version 0.13.1 \citep{ranger2017}.
For penalized likelihood methods, we use \pkg{glmnet} version 4.1.1
\citep{Friedman2010,Simon2011}.
The \pkg{SimDesign} package version 2.7 is used to set up simulation scenarios
\citep{Chalmers2020}.
