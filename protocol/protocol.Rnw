...
superior compared with methods of comparable complexity. Throughout this protocol
we will make a serious attempt at evaluating \ainet{} using state-of-the-art
simulation study methodology \citep{Morris2019}. After conducting simulations
as described in this protocol, we will start to apply questionable research practices
in order to illustrate how easy one can become overoptimistic and report a flawed
method as superior.
...
since any variable which leads to a decrease in model performance index should be
penalized equally.
...
\begin{itemize}
   ...
   \item \textbf{Adaptive LASSO} \citep{Zou2006}, with penalized loss function
