...
statistical superiority of any method, if we have enough flexibility in
changing the simulation conditions and evaluations. We propose a mock-method
termed \ainet{} which superficially seems reasonable and whose evaluation could actually be
published in a statistics journal. However, as we will explain in Section~\ref{sec:methods},
there are theoretical disadvantages which imply that the method should not be
superior compared with methods of equal complexity. Throughout this protocol
we will make a serious attempt at evaluating \ainet{} using state-of-the-art
simulation study methodology \citep{Morris2019}. After the serious simulations
have been conducted, we will then start to apply questionable research practices
in order to illustrate how easy one can become overoptimistic.

\section{Introduction}
\label{sec:introduction}
\todo{Disclaimer that this is a mock method and this paper is more about
the process of sim-hacking}
...
convergence problems and if so, the simulation parameters may be modified (Section~\ref{sec:exceptions}). Also this milestone will be time\-stamped.
...
There are arguments for both approaches, but we will use the second one
since any variable which leads to an increase in the Gini index should be
penalized equally.
