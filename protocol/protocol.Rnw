...
# X <- matrix(rnorm(4000*2000), nrow = 2000, ncol = 4000)
# Y <- rbinom(n = 2000, size = 1, prob = plogis(X%*%rep(c(0, 1), c(4000-15, 15))))
# rf <- randomForest(x = X, y = factor(Y), mtry = 15)
@
...
reverse to determine the dimensionality of the parameters in the model below.
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    $$\LS = - B^{-1} \sum_{b=1}^B n^{-1} \sum_{i=1}^n \left\{ y_{ib} \log(\hat{y}_{ib})
