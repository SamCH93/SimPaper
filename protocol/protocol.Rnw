...
gives values above 100 or below 2, they will be removed. This is done because in practice prediction models are only multivariable models ($p \geq 2$), but at the same time the number of predictors is rarely larger than 100
\citep{Kreuzberger2020,Seker2020,Wynants2020}, except in studies considering complex data, such as images, which are not the focus here. $\EPV \in \{\Sexpr{EPV}\}$ are chosen to cover scenarios with small to large number of covariates \citep[\cf][]{vanSmeden2018}.
