...
will therefore define the dimensionality $p$ via events per variable (EPV) by
$$p = \frac{n \cdot \prev}{\EPV}$$ and $2 \leq p \leq 100.$ If the above formula
gives values above 100 or below 2, they will be replaced by either $100$ or $2$.
This is done because in practice prediction models are only multivariable models
($p \geq 2$), but at the same time the number of predictors is rarely larger
than 100 (except for structured data, such as images, which are not the focus
here). $\EPV \in \{\Sexpr{EPV}\}$ are chosen to cover scenarios with small to
large number of covariates \citep[\cf][]{vanSmeden2018}.
...
errors, warnings, and any amendment that were made to the protocol.
...
this simulation conditions and just exclude the not-converged observations and
...
\hline
\bf Deviation & \bf Justification \\
\hline
Remove high-dimensional conditions & We decided to remove all conditions involving
...
Design not fully factorial & The above implies that the design is no longer fully
factorial. \\
\hline
\end{tabular}
\end{table}
