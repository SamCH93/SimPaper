...
## This script contains the code to produce figure 2
...
# load intermediate simulation results
if (repro_type == "figure") {
    folders <- c(file.path("..", "results-simulations", "pre-registered",
                           "results-anova"),
                 file.path("..", "results-simulations", "tweaked",
                           "results-anova"))
} else if (repro_type %in% c("full", "partial")) {
    folders <- c(file.path("..", "reproduce-simulations", "results_anova-full"),
                 file.path("..", "reproduce-simulations", "results_anova-nonlin"))
} else {
    stop("Supplied type of reproducibility not implemented.")
}
paths <- expand_grid(folder = folders, metric = which)
paths$path <- file.path(paths$folder, paste0("anova_", paths$metric, ".csv"))
pdat <- paths %>%
    mutate(d = map(path, ~ read_csv(.x, show_col_types = FALSE))) %>%
    unnest(c(d)) %>%
    mutate(set = factor(folder, levels = folders, labels = tlabs)) %>%
    dplyr::select(-path, -folder)
...
## plot simulation results after QRP E2 modifying the DGP: adding nonlinear
## effect and sparsity to DGP ("nonlinear")
pdatE2 <- pdatSubset %>%
...
pdatE2wide <- pdatE2 %>%
    ...
    dplyr::select(contrast, n, EPV, prev, rho, sparsity, Estimate, lwr, upr, set) %>%
...
pdatE3wide <- pdatE3 %>%
    ## creating variable to indicate whether modified DGP improved performance
    mutate(sparsity = 0.9) %>% # overwrite sparsity column
    dplyr::select(contrast, n, EPV, prev, rho, sparsity, Estimate, lwr, upr, set, alp, alp1) %>%
    pivot_wider(names_from = set, values_from = c(Estimate, lwr, upr)) %>%
    mutate(better = ifelse(Estimate_nonlinear < Estimate_final, "better", "worse"),
           from = ifelse(better  == "better", lwr_final, lwr_nonlinear),
           to = ifelse(better  == "better", upr_nonlinear, upr_final),
...
xmin <- min(c(p1$data$lwr, p3$data$lwr))*0.95
xmax <- max(c(p1$data$upr, p3$data$upr))*1.05
ggarrange(
    p0 + labs(subtitle = "Pre-registered") + coord_cartesian(xlim = c(xmin, xmax)),
    p1 + labs(subtitle = "QRP: E2") + coord_cartesian(xlim = c(xmin, xmax)),
    p2 + labs(subtitle = "QRP: E2 + E3") + coord_cartesian(xlim = c(xmin, xmax)),
    p3 + labs(subtitle = "QRP: E2 + E3 + R2 (reported)") +
    coord_cartesian(xlim = c(xmin, xmax)),
    common.legend = TRUE, ncol = 1, legend = "right"
)
ggsave("figure2.pdf", height = 13 * 0.9, width = 10 * 0.95)
