...
regression model, it is of no surprise that for reasonably large sample sizes,
...
\ainet{} and elastic net. The following pages show Figures of all estimands
which will now discuss one by one.
...
unscaled Brier scores from Figure~\ref{fig:tiebrier}. Therefore, conclusions are
...
non-convergence of the method itself.
...
We see that logistic regression (GLM) shows on average optimal calibration slope
